...
public class OperatingSystem {
     ...
     * implementiert ist!):
     */
    private int MAX_NUM_OF_PROCESSES = RAM_SIZE
    ...
    public synchronized int write(int pid, int virtAdr, int item) {
        int returnValue = -1; // default: Fehler
        ...
        if ((virtAdr < 0) || (virtAdr > VIRT_ADR_SPACE - WORD_SIZE)) {
            ...
            testOut("OS: write " + pid + " " + virtAdr + " " + item
                    + " +++ Seitennr.: " + virtualPageNum + " Offset: "
                    + offset);
            ...
            if (pte == null) {
                ...
                testOut("OS: write " + pid + " +++ Seitennr.: "
                        + virtualPageNum + " in Seitentabelle nicht vorhanden");
    ...
    public synchronized int read(int pid, int virtAdr) {
        int returnValue = -1; // default: Fehler
        int virtualPageNum; // Virtuelle Seitennummer der virt. Seite
        int offset; // Offset innerhalb der Seite
        int realAddressOfItem; // Reale Adresse des Datenworts
        Process proc; // Aktuelles Prozessobjekt
        PageTableEntry pte; // Eintrag für die zu schreibende Seite
        int item; // Zurückgegebenes Datenwort

        // Seitenadresse berechnen
        virtualPageNum = getVirtualPageNum(virtAdr);
        offset = getOffset(virtAdr);
        testOut("OS: read " + pid + " " + virtAdr + " +++ Seitennr.: "
                + virtualPageNum + " Offset: " + offset);

        // Seite in Seitentabelle referenzieren
        proc = getProcess(pid);
        pte = proc.pageTable.getPte(virtualPageNum);

        // Seite in Seitentabelle vorhanden? - Nein: Zugriffsfehler (Adresse
        // ungültig)!!
        if (pte == null) {
            // Seite nicht vorhanden:
            testOut("OS: read ERROR " + pid + " +++ Seitennr.: "
                    + virtualPageNum + " in Seitentabelle nicht vorhanden");
        } else {
            // Seite valid (im RAM)?
            if (!pte.valid) {
    ...
    private int getNewRAMPage(PageTableEntry newPte, int pid) {
        ...
        if (proc.pageTable.getSize() < MAX_RAM_PAGES_PER_PROCESS) {
            ...
            // hier: IMMER zurückschreiben, weil keine Kopie auf der Platte bleibt
    ...
    private void writeToRAM(int ramAdr, int item) {
        physRAM.put(Integer.valueOf(ramAdr), Integer.valueOf(ramAdr));
    }
    ...
    private int readFromRAM(int ramAdr) {
        ...
        itemObject = (Integer) physRAM.get(Integer.valueOf(ramAdr));
        if (itemObject == null) {
            result = -1;
        } else {
    ...
    private void dataTransferToDisk(int ramAdr, int diskAdr) {
        ...
        for (ri = ramAdr; ri < ramAdr + PAGE_SIZE; ri = ri + WORD_SIZE) {
            currentWord = (Integer) physRAM.get(Integer.valueOf(ri));
            physDisk.put(Integer.valueOf(di), currentWord);
            di = di + WORD_SIZE;
        }
    }
    ...
    private void dataTransferFromDisk(int diskAdr, int ramAdr) {
        ...
        for (di = diskAdr; di < diskAdr + BLOCK_SIZE; di = di + WORD_SIZE) {
            currentWord = (Integer) physDisk.get(Integer.valueOf(di));
            physRAM.put(Integer.valueOf(ri), currentWord);
            ri = ri + WORD_SIZE;
        }
    }
    ...
    private void freeRAMPage(int ramAdr) {
        ...
        nullWord = Integer.valueOf(0);
        for (ri = ramAdr; ri < ramAdr + PAGE_SIZE; ri = ri + WORD_SIZE) {
            physRAM.put(Integer.valueOf(ri), nullWord);
        }
    ...
    private void freeDiskBlock(int diskAdr) {
        ...
        nullWord = Integer.valueOf(0);
        for (di = diskAdr; di < diskAdr + BLOCK_SIZE; di = di + WORD_SIZE) {
            physDisk.put(Integer.valueOf(di), nullWord);
        }
        // In Freibereichsliste eintragen
    ...
    /**
     * @return Anzahl Operationen innerhalb eines lokalen Adressbereichs
     */
    ...
    /**
     * @param i Anzahl Operationen innerhalb eines lokalen Adressbereichs
     */
